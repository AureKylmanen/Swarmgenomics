...
Remember to ```chmod +x numt_detection.bash```. Then run with:
```
./numt_detection.bash "species" "path/to/reference_genome.fna.gz" "params.txt"
```
The results will be copied into the results directory within the species directory.

## Numt detection step-by-step
