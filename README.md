...
One option for running SwarmGenomics is through the de.NBI Cloud (https://cloud.denbi.de/), which provides access to high-performance computing resources for bioinformatics projects. After setting up an account and requesting resources, you can connect to your virtual machine (VM) using **PuTTY** and **FileZilla**:
- PuTTY is used to establish a SSH connection to the VM. This lets you log in to the remote machine, install packages, and run SwarmGenomics directly on the de.NBI infrastructure.
- FileZilla is used to transfer files between your local computer and the VM. This makes it straightforward to upload scripts or download results generated by your analyses.
