...
## What you need
The input for Kraken2 is a **FASTQ file**, generated from a BAM file containing unmapped reads (either from the pipeline or from a user-provided BAM file, as described below).

### Installations
You will need to download kreport2krona.py script. As well as Kraken2 using miniconda with following steps:
...
```
# Create a BAM file with unmapped reads if not already created 
